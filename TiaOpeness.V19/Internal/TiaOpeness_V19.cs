...
{
    class TiaOpeness_V19 : ITiaOpeness
    {
        private const string PROJECTEXTENSION = ".ap19";
        private const string ARCHIVEEXTENSION = ".zap19";
        ...
        public bool OpenProject(string filePath)
        {
            if (!IsValidProjectFile(filePath))
                throw new FileNotFoundException($"File '{filePath}' is not a valid project file for TIA Portal V19");
        ...
        public bool OpenArchivedProject(string filePath, string destinationDir)
        {
            if (!IsValidProjectArchive(filePath))
                throw new FileNotFoundException($"File '{filePath}' is not a valid project archive for TIA Portal V19");
