...
namespace TiaOpeness.V21.Internal
{
    class TiaOpeness_V21 : ITiaOpeness
    {
        private const string PROJECTEXTENSION = ".ap21";
        private const string ARCHIVEEXTENSION = ".zap21";
        ...
        public bool OpenProject(string filePath)
        {
            if (!IsValidProjectFile(filePath))
                throw new FileNotFoundException($"File '{filePath}' is not a valid project file for TIA Portal V21");
            ...
            else
            {
                ...
                var simulationSettingsProvider = project.GetService<PlcSimulationSettingsProvider>();
                var simulationEnabled = simulationSettingsProvider.IsSimulationDuringBlockCompilationEnabled;
                LogProject(project, simulationEnabled);
                if (!simulationEnabled)
                {
                    logger.Info("Enabling simulation on project");
                    simulationSettingsProvider.IsSimulationDuringBlockCompilationEnabled = true;
                }
                result = true;
            }
            return result;
        ...
        public bool OpenArchivedProject(string filePath, string destinationDir)
        {
            if (!IsValidProjectArchive(filePath))
                throw new FileNotFoundException($"File '{filePath}' is not a valid project archive for TIA Portal V21");
        ...
        private static void LogProject(Project project, bool simulationEnabled)
        {
            ...
            logger.Verbose(
                 $"{LOGTAB}Simulation enabled: {simulationEnabled}"
            );
